...
const ServicesBody = () => {
  ...
  return (
    <div className="h-full pt-14 px-14 w-full">
      <h1 className="font-medium text-3xl">
        We are <span className="text-red-600 font-bold">Dedicated</span> to{" "}
        <br /> serve you all time
      </h1>
      <div className="h-3/4 w-full grid md:grid-cols-3 grid-cols-1 gap-5 sm:grid-cols-2 mt-10 flex-wrap">
        {data.map((items, index) => (
          <div key={index}>

            <div
              key={index}
              className="group h-80 w-full flex flex-col border rounded-md bg-[#F5F7FA] p-4"
            >
